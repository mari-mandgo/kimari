...
export type Meeting = {
  ...
  calls?: CallMeta[];
  /**
   * 拾い出しの結果。項目の見出しをキーにする。
   *
   * 一度出したものを持っておくと、開き直すたびに同じ内容でLLMを呼ばずに済む。
   * 公開デモでは特に効く（待ち時間ゼロで出せて、原価もかからない）。
   * 型は any ではなく TakeoffResponse だが、循環参照を避けるため構造だけを持つ。
   */
  takeoffs?: Record<string, unknown>;
  /**
   * この打ち合わせが工事のどの段階だったか。
